...
bun x convex deploy       # deploy to self-hosted backend
```
...
- `api` imported from `@/lib/convex` (uses `anyApi` until `bun x convex dev` generates typed bindings)
...
- `bins/convex-backend/convex/_generated/` — auto-generated by `bun x convex dev`, do not edit manually
