...
test.describe("Contacts — group chat filter", () => {
	...
	test("scenario 3: group chat tab appears but merged timeline excludes it", async ({
		...
		await expect(
			tablist.getByRole("tab", { name: /all dialogs/i }),
		).toBeVisible();
		await expect(
			tablist.getByRole("tab", { name: /Grace Hopper/ }),
		).toBeVisible();
		await expect(
		...
		await expect(activeTabPanel.locator("text=GROUP_ONLY_MESSAGE")).toHaveCount(
			0,
		);
