...
export const create = humanMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.initialLink) {
      ...
      const chat = await ctx.db
        .query("chats")
        .withIndex("by_chatId", (q) => q.eq("chatId", args.initialLink!.chatId))
        .unique();
      ...
      const existing = await ctx.db
        ...
        .withIndex("by_userId_chatId_senderId", (q) =>
          q
            .eq("userId", ctx.caller.tokenIdentifier)
            .eq("chatId", args.initialLink!.chatId)
            .eq("senderId", args.initialLink!.senderId)
        )
        .unique();
      ...
      if (existing) {
        ...
        await ctx.db.insert("chatContactLinks", {
          userId: ctx.caller.tokenIdentifier,
          chatId: args.initialLink.chatId,
          senderId: args.initialLink.senderId,
          contactId,
          createdAt: now,
        });
      }
...
interface MergedCursor {
  perChat: Record<
    string,
    { timestamp: number; messageId: string } | null
  >;
}
